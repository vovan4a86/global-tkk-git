...
class City extends Model {
	...
	public function generateIndexPage(): Page {
        ...
        $page->name = $this->name;
        $page->title = $this->title != '' ? $this->title: Settings::get('city_index_title');
        $page->h1 = $page->h1 != '' ? $page->h1 : Settings::get('city_index_h1');
        $page->keywords = $this->keywords;
        $page->description = $this->description;
        $page->text = $this->text != '' ? $this->text : Settings::get('region_text');
        return $page;
	}

    public static function current($request, $city_alias = null, $remember = true)
    {
        ...
        if (Request::path() == '/') {
            if ($city_alias = session('city_alias')) {
                ...
                if (!$first_visit) { //если не первый визит
                    ...
                    View::share('showPopupChangeCity', true);
...
//                        $city = City::whereAlias('ekb')->first();
