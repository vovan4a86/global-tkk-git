...
class SiteHelper {
	 ...
	 * @return String
	 */
	public static function getNumEnding(int $number, array $endingArray = ['товар', 'товара', 'товаров']): string
    {
		$number = $number % 100;
	...
	private static function recurseAddCatalog(&$map, $parent = null) {
		...
		foreach ($catalogs as $catalog) {
			...
			foreach ($products as $product) {
				$map->add_url($catalog_url . '/' . $product->alias);
				self::$urls[] = $catalog_url . '/' . $product->alias;
			}
